...
public protocol ClusterManagerDelegate: class {
    /**
     ...
     - Returns: The cell size at the given zoom level. If you return nil,
       the cell size will automatically adjust to the zoom level.
     */
...
open class ClusterManager {
    ...
    /**
     ...
     - Parameters:
        - mapView: The map view object to reload.
        - completion: A closure to be executed when the reload finishes.
     The closure has no return value and takes a single Boolean argument that
     indicates whether or not the reload actually finished before the completion handler was called.
     */
        ...
        dispatchQueue.async(flags: .barrier) {
            for value in hash.values where value.count > 1 {
                for (index, annotation) in value.enumerated() {
                    ...
                    (annotation as? MKPointAnnotation)?.coordinate = annotation.coordinate.coordinate(onBearingInRadians: bearing,
    ...
    func coordinate(annotations: [MKAnnotation],
        ...
        case .average:
            ...
            let totals = coordinates.reduce((latitude: 0.0,
                                             longitude: 0.0)) { ($0.latitude + $1.latitude,
                                                                                $0.longitude + $1.longitude) }
            return CLLocationCoordinate2D(latitude: totals.latitude / Double(coordinates.count),
    ...
    func mapRects(zoomScale: Double, visibleMapRect: MKMapRect) -> [MKMapRect] {
        ...
        for xValue in minX...maxX {
            for yValue in minY...maxY {
                var mapRect = MKMapRect(x: Double(xValue) / scaleFactor, y: Double(yValue) / scaleFactor, width: 1 / scaleFactor, height: 1 / scaleFactor)
